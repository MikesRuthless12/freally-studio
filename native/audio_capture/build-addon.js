/**
 * Build script for the native audio capture addon.
...
console.log(`[audio_capture] Building audio capture addon (${buildType})...`);
if (cmakePath) console.log(`[audio_capture] Using CMake: ${cmakePath}`);

try {
    execSync(cmd, { cwd: __dirname, stdio: 'inherit' });
    console.log('[audio_capture] Build succeeded.');
} catch (err) {
    console.error('[audio_capture] Build failed.');
    process.exit(1);
}
